...
@DEEPAK.on(events.NewMessage(pattern="^[?!/]pinned"))
async def get_pinned(event):
    chat_id = (str(event.chat_id)).replace("-100", "")

    Ok = await DEEPAK.get_messages(event.chat_id, ids=types.InputMessagePinned()) 
...
@DEEPAK.on(events.NewMessage(pattern="^[!?/]pin ?(.*)"))
@is_admin
async def pin(event, perm):
    ...
    if "notify" in input_str:
       await DEEPAK.pin_message(event.chat_id, msg, notify=True)
       return
    await DEEPAK.pin_message(event.chat_id, msg)   

@DEEPAK.on(events.NewMessage(pattern="^[!?/]unpin ?(.*)"))
@is_admin
...
@DEEPAK.on(events.NewMessage(pattern="^[!?/]permapin"))
@is_admin
async def permapin(event, perm):
    ...
    if not msg:
       ...
       return
    hn = await DEEPAK.send_message(event.chat_id, msg.message)
    await DEEPAK.pin_message(event.chat_id, hn, notify=True)


@DEEPAK.on(events.NewMessage(pattern="^[!?/]unpinall"))
...
"""
    ...
    await DEEPAK.send_message(event.chat_id, UNPINALL, buttons=[
    [Button.inline("Confirm", data="unpin")], 
    [Button.inline("Cancel", data="cancel")]])

@DEEPAK.on(events.callbackquery.CallbackQuery(data="unpin"))
async def confirm(event):
    check = await event.client.get_permissions(event.chat_id, event.sender_id)
    if check.is_creator:
        await DEEPAK.unpin_message(event.chat_id)
        await event.edit("Unpinned All Msgs!")
        return 

    await event.answer("Group Creator Required!")

@DEEPAK.on(events.callbackquery.CallbackQuery(data="cancel"))
...
@DEEPAK.on(events.callbackquery.CallbackQuery(data="pins"))
async def _(event):
